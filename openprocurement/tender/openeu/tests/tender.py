...
class BaseTenderEUResourceTest(object):
    ...
    def test_create_tender_invalid(self):
        ...
        self.assertIn({u'description': [u'This field is required.'], u'location': u'body', u'name': u'items'},
                      response.json['errors'])
        self.assertIn({u'description': [u'This field is required.'], u'location': u'body', u'name': u'enquiryPeriod'},
                      response.json['errors'])
        ...
        self.assertEqual(response.json['status'], 'error')
        self.assertEqual(response.json['errors'], [
            {u'description': {u'endDate': [u'date value out of range']}, u'location': u'body',
             u'name': u'enquiryPeriod'}
        ])
        ...
        test_tender_data['auctionPeriod'] = {'startDate': (now + timedelta(days=35)).isoformat(),
                                             'endDate': (now + timedelta(days=35)).isoformat()}
        test_tender_data['awardPeriod'] = {'startDate': (now + timedelta(days=34)).isoformat(),
        ...
        self.assertEqual(response.json['errors'], [
            {u'description': [{u'deliveryDate': [u'This field is required.'],
                               u'deliveryAddress': {u'postalCode': [u'This field is required.'],
                                                    u'locality': [u'This field is required.']}}], u'location': u'body',
             u'name': u'items'}
        ])
    ...
    def test_invalid_bid_tender_lot(self):
        ...
        response = self.app.post_json('/tenders/{}/bids'.format(tender_id),
                                      {'data': {'selfEligible': True, 'selfQualified': True,
                                                'status': 'draft',
                                                'lotValues': [{"value": {"amount": 500}, 'relatedLot': i} for i in
                                                              lots],
...
class TenderEUResourceTest(BaseTenderWebTest, BaseTenderResourceTest, BaseTenderOpenUAResourceTest,
                           BaseTenderEUResourceTest):
...
class TenderProcessTest(BaseTenderWebTest):
    ...
    def test_invalid_tender_conditions(self):
        ...
        response = self.app.post_json('/tenders/{}/complaints'.format(tender_id),
                                      {'data': {'title': 'invalid conditions', 'description': 'description',
                                                'author': complaint_author, 'status': 'claim'}})
        complaint_id = response.json['data']['id']
        complaint_owner_token = response.json['access']['token']
        # answering claim
        self.app.patch_json('/tenders/{}/complaints/{}?acc_token={}'.format(tender_id, complaint_id, owner_token),
                            {"data": {
                                "status": "answered",
        ...
        self.app.patch_json(
            '/tenders/{}/complaints/{}?acc_token={}'.format(tender_id, complaint_id, complaint_owner_token), {"data": {
                "satisfied": True,
                "status": "resolved"
            }})
        # cancellation
    ...
    def test_one_qualificated_bid_tender(self):
        ...
        response = self.app.patch_json('/tenders/{}/qualifications/{}'.format(tender_id, qualifications[0]['id']),
    ...
    def test_multiple_bidders_tender(self):
        ...
        self.assertEqual(response.json['data']['participationUrl'],
